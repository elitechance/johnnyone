...
- **The web app now wears the 2026-07 mock**: a global **icon nav rail** (J1 logo · `+ New` · Work / Files / Shells / Settings) replaces the old text sidebar (with a mobile bottom-nav from the same list), and **"Work"** is a unified **initiative console** at the existing `/terminal` route — a master-detail grid wrapping the existing Transcript / Raw terminal / Plan / Diff pane shell with an initiative master-list, a **lifecycle bar** + status pills rendering the P1 `initiativeStatus`/`health` in per-stage colors, and a right column (validation-lens summary + touched host files). The accent flips **blue → amber** app-wide via the token layer (both light + dark palettes authored to the mock; dark stays active). Web + `ui` only — no Rust/worker/relay edit, no new route or GraphQL op, no new dependency; every surface reuses the P1–P7 components. See [`docs/app-shell.md`](docs/app-shell.md).
- **Console fix-forward** (four findings from real use of the mock): the **Transcript / Raw terminal / Plan / Diff** tabs are now the **primary per-initiative surface**, present whether or not a terminal is attached (the Raw tab holds an inline attach affordance); the **Plan tab is built** (doc navigator + rendered plan markdown + phase cards with per-task status, from `getAgentPlan` + `readFile` via the render core); the **validation column shows a "default template" vs "configured" badge** so per-initiative validation reads correctly (the runtime was already per-initiative — verified, no Rust change); a **raw shell / attached tmux opens a plain terminal surface** (no initiative chrome) and the tmux-attach de-dupe bug is fixed; and the console gains **resizable rail↔center↔validation dividers** with widths persisted. Web + `ui` only, reuse-only. See [`docs/console-fixes.md`](docs/console-fixes.md).
- **The SDLC loop is gated on evidence, not on the agent's word.** A `ready` report now clears cheap mechanical checks *before* a review round is spent: the planner's acceptance criteria must name what decides them (a command, a route + element, a `file:line`), declared `test_commands` are run by the coordinator itself, spec lines removed from the plan store during development are rejected, and screenshots must be distinct and described. A fourth verdict, **`PASS_WITH_FOLLOWUPS`**, lets a lens record a non-blocking observation without costing a round — and the roll-up preserves it, so follow-ups travel with the work instead of being laundered into a clean pass. Two budgets bound the loop: ready reports bounce at most 3 times per stage attempt, and review rounds stop at 6 or as soon as the findings stop moving. See [`docs/sdlc-gates.md`](docs/sdlc-gates.md).
...
- **`docs/validation-diff.md`** — configurable validation + the Diff tab (overhaul
  ...
  the render core
- **`docs/app-shell.md`** — the web-app shell (overhaul P8): the amber token
  retarget (blue→amber `--jo-accent` + `--jo-st-*` palette, dormant light theme),
  the pure `lifecycle-status` map + the `johnny-lifecycle-bar`/`johnny-status-pill`
  primitives, the icon nav rail + mobile bottom-nav from one `nav-items` list, and
  the initiative console assembled at `/terminal` via the pure `console-logic`
  projection — layout/theme only, reusing the P1–P7 components
- **`docs/sdlc-gates.md`** — the SDLC gates: the four-verdict set and how a
  roll-up preserves `PASS_WITH_FOLLOWUPS`, the six ready gates that run before a
  review round is spent (executable acceptance, coordinator-run tests, spec
  weakening, evidence provenance, task reconciliation, deliverable presence), and
  the two loop guards (findings-diff convergence, ready-bounce cap)
- **`docs/console-fixes.md`** — the console fix-forward: the Transcript/Raw/Plan/Diff
  tabs made the primary per-initiative surface (present without a terminal), the
  built-out Plan tab (doc nav + phase cards over `getAgentPlan`), the
  default-template vs configured validation badge, the plain shell surface +
  tmux-attach de-dupe fix, and the resizable console dividers — web + `ui` only
...
| Web-app shell — nav rail + initiative console + lifecycle bar + amber theme (overhaul P8) | Done (2026-07) |
