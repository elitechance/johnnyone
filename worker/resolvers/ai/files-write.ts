...
// the host owns behavior; the worker only relays, gated by the files:write scope. `content` is
// never logged.
export default async function filesWrite(
  _parent: unknown,
  args: { path: string; content: string; encoding?: string | null },
  ctx: ResolverContext,
) {
  await authorizeForAltToken(ctx, 'files:write');
  return desktopRpc<unknown>(ctx, 'files_write', {
    path: args.path,
