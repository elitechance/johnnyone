...
export default async function cancelToolExecution(
  parent: unknown,
  args: { id: string },
  ctx: ResolverContext,
) {
  ...
  const cancellableStatuses = ['pending', 'approved', 'running'];
  if (!cancellableStatuses.includes(existing.status)) {
    throw new Error(`Cannot cancel tool execution with status: ${existing.status}`);
  }

  await ctx.db
    .prepare(
      `UPDATE ai_tool_executions SET status = 'cancelled', updated_at = datetime('now') WHERE id = ? AND tenant_id = ?`,
    )
    .bind(args.id, ctx.auth.tenantId)
    .run();

  // Notify the AgentSessionDO that the tool was cancelled
  const doId = ctx.env.AGENT_SESSION_DO.idFromName(existing.session_id);
  const stub = ctx.env.AGENT_SESSION_DO.get(doId);

  stub
    .fetch(
      new Request('https://do.internal/tool-cancelled', {
        method: 'POST',
    ...
    .catch((err: Error) => {
      console.error('Failed to notify DO of tool cancellation:', err);
    });
  ...
  if (!execution) {
    throw new Error('Failed to cancel tool execution');
  }
